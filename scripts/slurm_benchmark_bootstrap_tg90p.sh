...
SAFE_TILE_MEMORY="${SAFE_TILE_MEMORY:-}"
SAFE_TILE_CELLS="${SAFE_TILE_CELLS:-}"
CACHE_DIR="${CACHE_DIR:-}"
CACHE_KEY="${CACHE_KEY:-}"
...
if [ -n "$BOOTSTRAP_MODE" ]; then
  CMD+=(--bootstrap-mode "$BOOTSTRAP_MODE")
fi

if [ -n "$SAFE_TILE_MEMORY" ]; then
  CMD+=(--safe-tile-memory "$SAFE_TILE_MEMORY")
fi

if [ -n "$SAFE_TILE_CELLS" ]; then
  CMD+=(--safe-tile-cells "$SAFE_TILE_CELLS")
fi
